...
export class UsageService {
  async create_usage(app_id, usage_data) {
    try {
      ...
      const usage = await prisma.usage.create({
        data: {
          ...
          period_start: usage_data.period_start ? new Date(usage_data.period_start) : null,
          period_end: usage_data.period_end ? new Date(usage_data.period_end) : null,
  ...
  async create_batch_usage(app_id, records) {
    try {
      ...
      const usage_records = records.map(record => ({
        ...
        quota_plan_id: record.quota_plan_id,
        metric_name: record.metric_name,
        quantity: record.quantity,
        timestamp: record.timestamp ? new Date(record.timestamp) : new Date(),
        period_start: record.period_start ? new Date(record.period_start) : null,
        period_end: record.period_end ? new Date(record.period_end) : null,
        carried_over_from_period: record.carried_over_from_period,
  ...
  async get_usage(app_id, options = {}) {
    try {
      const { 
        limit = 50, 
        offset = 0, 
  ...
  async get_usage_summary(app_id, options = {}) {
    try {
      const { 
        customer_id, 
        metric_name, 
  ...
  // New method to get usage for a specific period (for quota checking)
  async get_usage_for_period(app_id, customer_id, metric_name, period_start, period_end) {
    try {
  ...
  // New method to create carry-over usage records
  async create_carry_over_usage(app_id, customer_id, metric_name, carry_over_quantity, new_period_start, new_period_end, previous_period_id) {
    try {
      const carry_over_usage = await prisma.usage.create({
        data: {
          ...
          customer_id,
          metric_name,
          quantity: -carry_over_quantity, // Negative to indicate additional quota
          timestamp: new Date(),
